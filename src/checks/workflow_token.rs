...
pub const HOW_TO_FIX: &str = "github → organization → settings → actions → general → workflow permissions → read repository contents and packages permissions (and the same setting at the repository level).";
